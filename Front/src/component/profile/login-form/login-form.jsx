...
import { useOscar } from "@/component/oscar/oscar-context/oscar-context";

function LoginForm({ onLogin }) {
  const [step, setStep] = useState("email");
  const [loginType, setLoginType] = useState("passKey");
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [code, setCode] = useState("");
  const [timer, setTimer] = useState(30);
  const { showOscar } = useOscar();
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        showOscar("Welcome to your account");
        onLogin?.(data);
      } else {
        alert(data.message || data);
      }
    ...
    if (step === "email" && email) {
      try {
        ...
        if (data.success) {
          setStep("code");
          setTimer(30);
          showOscar("Verification code sent");
        } else {
  ...
  const handleResend = async () => {
    ...
    showOscar("Verification code sent again");
    setTimer(30);
  };
    ...
    <div className="new-auth">
      ...
      <div className="login-section">
        <div className="login-btn-list">
          {loginType !== "google" && (
            <div className="login-btn" onClick={() => setLoginType("google")}>
              <div>
          ...
          {loginType !== "code" && (
            <div className="login-btn" onClick={() => setLoginType("code")}>
              <div>
          ...
          {loginType !== "passKey" && (
            <div className="login-btn" onClick={() => setLoginType("passKey")}>
              <div>
          ...
          {loginType !== "register" && (
            <div className="login-btn" onClick={() => setLoginType("register")}>
              <div>
                <svg aria-hidden="true" viewBox="0 0 18 18">
                  <path d="M9 1.5a4 4 0 0 0-4 4c0 1.26.59 2.38 1.5 3.11A6.5 6.5 0 0 0 2.5 14.6c0 .5.4.9.9.9h7.15A4.5 4.5 0 0 1 14 8.25c-.4.1-.74.16-1 .16a4 4 0 0 0-4-6.91Zm0 1.5a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5Zm0 6.5c.78 0 1.5.16 2.14.45a4.5 4.5 0 0 0-.9 4.05H4.08A5 5 0 0 1 9 9.5Zm5 1a.75.75 0 0 0-.75.75v1.5h-1.5a.75.75 0 0 0 0 1.5h1.5v1.5a.75.75 0 0 0 1.5 0v-1.5h1.5a.75.75 0 0 0 0-1.5h-1.5v-1.5A.75.75 0 0 0 14 10.5Z" />
                </svg>
              </div>
